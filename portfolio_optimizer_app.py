...
def generate_pdf_report(efficient_portfolios, max_sharpe_idx, min_vol_idx, filtered_tickers, filtered_info):
    """Generate PDF report"""
    buffer = io.BytesIO()
    ...
    story.append(Paragraph(f"Analysis performed on {len(filtered_tickers)} stocks that met the specified criteria.", styles['Normal']))
    story.append(Spacer(1, 12))
    
    # Optimal Portfolios
    if efficient_portfolios and max_sharpe_idx is not None and min_vol_idx is not None:
        max_sharpe = efficient_portfolios[max_sharpe_idx]
        min_vol = efficient_portfolios[min_vol_idx]
        
        story.append(Paragraph("Optimal Portfolio Results", styles['Heading2']))
        
        # Max Sharpe Portfolio
        story.append(Paragraph("Maximum Sharpe Ratio Portfolio:", styles['Heading3']))
        story.append(Paragraph(f"Expected Return: {max_sharpe['return']:.2%}", styles['Normal']))
        story.append(Paragraph(f"Volatility: {max_sharpe['volatility']:.2%}", styles['Normal']))
        story.append(Paragraph(f"Sharpe Ratio: {max_sharpe['sharpe']:.3f}", styles['Normal']))
        story.append(Spacer(1, 6))
        
        # Min Vol Portfolio
        story.append(Paragraph("Minimum Volatility Portfolio:", styles['Heading3']))
        story.append(Paragraph(f"Expected Return: {min_vol['return']:.2%}", styles['Normal']))
        story.append(Paragraph(f"Volatility: {min_vol['volatility']:.2%}", styles['Normal']))
        story.append(Paragraph(f"Sharpe Ratio: {min_vol['sharpe']:.3f}", styles['Normal']))
        story.append(Spacer(1, 12))
    
    # Stock Analysis Table
    story.append(Paragraph("Stock Analysis", styles['Heading2']))
    table_data = [['Ticker', 'Market Cap (B)', 'Avg Volume (M)', 'Sector']]
    
    for ticker in filtered_tickers:
        info = filtered_info[ticker]
        table_data.append([
            ticker,
            f"${info['market_cap']:.1f}B",
            f"{info['avg_volume']:.1f}M",
            info['sector']
        ])
...
def main():
    ...
    if getattr(st.session_state, 'analysis_complete', False):
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📈 Efficient Frontier", "📊 Portfolio Analysis", "📋 Stock Details", "📄 Reports"])
        ...
        with tab4:
                ...
                if st.button("📄 Generate PDF Report"):
                    if hasattr(st.session_state, 'efficient_portfolios'):
                        pdf_buffer = generate_pdf_report(
                            ...
                            st.session_state.max_sharpe_idx,
                            st.session_state.min_vol_idx,
                            st.session_state.filtered_tickers,
                            st.session_state.filtered_info
                        )
            ...
            with col2:
                if hasattr(st.session_state, 'efficient_portfolios') and st.button("📊 Download Chart (HTML)"):
                    fig = create_efficient_frontier_plot(
                        st.session_state.efficient_portfolios,
                        st.session_state.max_sharpe_idx,
                        st.session_state.min_vol_idx,
                        st.session_state.filtered_tickers,
                        st.session_state.optimizer.returns
                    )
                    ...
                    st.download_button(
                        ...
                        data=html_str,
                        file_name=f"efficient_frontier_{datetime.now().strftime('%Y%m%d')}.html",
                        mime="text/html"
                    )
